...
def init_db():
    ...
    if "play_time" not in columns:
        cursor.execute(
            "ALTER TABLE players ADD COLUMN play_time TEXT"
        )
...
def update_status(user_id, status, play_time=None):
...
async def start(
    ...
    if not faceit_id:
        ...
        await update.message.reply_text(
            ...
            "Твой FACEIT-профиль ещё "
            "не привязан.\n\n"
            "Отправь ссылку на свой "
            "FACEIT-профиль."
        )
...
async def process_faceit(
    ...
    result, error = get_faceit_player(
        text
    )
...
async def change_status(
    ...
    context.user_data[
        "waiting_for_play_time"
    ] = True

    await update.message.reply_text(
        "🎮 Во сколько сегодня играешь?\n\n"
        "Напиши точное время в формате:\n"
        "21:30\n\n"
        "Например: 20:00"
    )
...
def get_faceit_player_stats(
    faceit_id,
    limit=100
):

    url = (
        f"https://open.faceit.com/data/v4/"
        f"players/{faceit_id}/games/cs2/stats"
...
async def text_handler(
    ...
    # Затем проверяем,
    # ждём ли FACEIT-профиль

    if context.user_data.get(
        "waiting_for_faceit"
    ):

        await process_faceit(
            update,
            context
        )
...
def main():
    ...
    print(
        "Бот запущен! VERSION: PLAY_TIME_TEST_1"
    )
